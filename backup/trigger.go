...
type TriggerFunc interface {
	Execute(db *Database, t *Table, column string, extraParameters []string, row1, row2 map[string]string) int
}
...
func ExecuteTrigger(db *Database, t *Table, r *Result, operation string, row1, row2 map[string]string) int {
	for column, _ := range row1 {
		raCopy := r.Copy()
		// Filter according to the column name and operation type.
		raCopy.MultipleSelect(Condition{Alias: "COLUMN", Filter: Eq{}, Parameter: column},
			Condition{Alias: "OP", Filter: Eq{}, Parameter: operation})
		// For each trigger.
