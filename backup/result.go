...
func (r *Result) Load(t *Table) (*Result, int) {
	_, exists := r.Tables[t.Name]
	if exists {
		return r, TableAlreadyExists
	}
	...
	for columnName, _ := range t.Columns {
		if !strings.HasPrefix(columnName, ThePrefix) {
			_, exists := r.Aliases[columnName]
			if exists {
				Warn("ra", "Load", "Column name "+columnName+" duplicates an existing alias")
			}
...
func (r *Result) Read(rowNumber int) (map[string]string, int) {
	...
	for _, column := range r.Aliases {
		...
		tableRow, status := Table.Read(RowNumbers[rowNumber])
		if status != OK {
			break
		}
