...
import (
	"github.com/prestonTao/database/database"
...
func New(db *database.Database) *Transaction {
	theID := time.Now().Nanosecond()
	return &Transaction{db, make([]Undoable, 0), strconv.Itoa(theID), int64(theID), make([]*table.Table, 0)}
}
